...
const AddNote = () => {
  ...
  const { selectedFinancialYear, accounts } = useAccountingStore();
  const { setNotes } = useNotesStore();
  const { addAlert } = useAlertStore();
  const [loading, setLoading] = useState(false);
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const linkedAccountId = queryParams.get("linkedAccount");
  const { financialStatement } = useFinancialStatementStore();

  const [formData, setFormData] = useState({
    title: "",
    description: "",
    linkedAccount: linkedAccountId || "",
    financialStatement: financialStatement || "pnl",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const noteData = {
      ...formData,
      financialYear: selectedFinancialYear._id,
    };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 bg-white/50"
            />
          </div>
